package main // pretty much a copy-paste of gliderlabs's docker ssh example but limited to one container
...
func dockerRun(cfg *container.Config, sess ssh.Session) (status int64, cleanup func(), err error) {
	...
	res, err := docker.ContainerCreate(ctx, cfg, hostcfg, nil, "")
	if err != nil {
		return
	}
	cleanup = func() {
